...
class _ScannerLocalisationState extends State<ScannerLocalisation>
  ...
  void initState() {
    ...
    _scaleAnimation = Tween(begin: 100.0, end: 350.0).animate(scannerAnimationController);
    _widthAnimation = Tween(begin: 2.5, end: 10.0).animate(scannerAnimationController);
    _opacityAnimation = Tween(begin: 1.0, end: 0.1).animate(scannerAnimationController);
    _colorAnimation =
        ColorTween(begin: Colors.white, end: Colors.lightBlue[100]).animate(
            CurvedAnimation(parent: scannerAnimationController, curve: Interval(0.0, 0.6)));
    _containerAnimation = Tween(begin: 0.0, end: 100.0).animate(
        CurvedAnimation(parent: scannerAnimationController, curve: Interval(0.6, 1.0)));
   // scannerAnimationController.forward();
    _timerUpdateSignal = Timer.periodic(Duration(seconds: 5), (_) => MQTTController.to.updateIntensity());
